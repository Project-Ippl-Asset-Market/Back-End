...
//   const { id } = req.params;
//   const { uid, email, firstName, lastName, role, username } = req.body;
...
//     // Cek apakah pengguna ada di Firebase Auth
...
//       const profileImageUrl = `https://storage.googleapis.com/${bucket.name}/${fileUpload.name}`;
...
// export const updateAdmin = async (req, res) => {
//   const { id } = req.params;
//   const { uid, email, firstName, lastName, role, username, password } =
//     req.body; // Tambahkan password
//   const file = req.file; // Ambil file dari req.file
...
//     if (password) {
//       updateAuthData.password = password; // Tambahkan password jika ada
//     }
...
//     if (file) {
//       const bucket = getStorage().bucket(); // Ambil bucket storage
//       const fileName = `profileImage-${uuidv4()}`; // Buat nama file unik
//       const fileUpload = bucket.file(`images-admin/${fileName}`); // Path yang sesuai
...
//       // Dapatkan metadata file setelah upload
//       const [metadata] = await fileUpload.getMetadata();

//       // Dapatkan token untuk URL
//       const token = metadata.metadata
//         ? metadata.metadata.firebaseStorageDownloadTokens
//         : uuidv4(); // Ambil token jika ada, jika tidak, buat token baru

//       // Dapatkan URL download dengan format yang diminta
//       profileImageUrl = `https://firebasestorage.googleapis.com/v0/b/${bucket.name}/o/images-admin%2F${fileName}?alt=media&token=${token}`;
...
//         const oldFileName = oldProfileImageUrl.split("/").pop().split("?")[0]; // Ambil nama file dari URL
